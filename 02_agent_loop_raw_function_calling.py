...
# Wrap plain functions as LangChain tools so they expose metadata like .name.
get_product_price = tool(get_product_price)
apply_discount = tool(apply_discount)


# Difference 2 : Without @tool, we must MANUALLY define JSON schema or each function.
# This is exactly what langchain's @tool  decorator automatically generates.
...
tools_for_llm = [
    {
        ...
        "function": {
            ...
            "description": "Look up the price of a product in a predefined catalog.",
            "parameters": {
                "type": "object",
                "properties": {
                    "product": {
                        "type": "string",
                        "description": "Name of the product. i.e. laptop, mouse, keyboard"
                    }
                },
                "required": ["product"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "discount_tier": {
                        "type": "string",
                        "description": "Apply Discount tier (bronze, silver, gold) to price and return the final price."
                    }
                },
...
# Difference 3: Without Langchain, we must MANUALLY parse the tool calls from the LLM's response for langsmith

@traceable(name='Ollama Chat',run_type='llm')
def ollama_chat_traced(messages):
    """
    Call Ollama Chat with tracing enabled.
    Args:
